...
If the predecessor pageflip occurs first, pageflip handling establishes and records the actual physical claim before binding. If rendering completes first, the next render-completion path binds against the recorded physical anchor. Both paths validate output generation and the exact predecessor transaction/frame/token identity. A mismatch abandons the unbound transaction without creating a claim.
