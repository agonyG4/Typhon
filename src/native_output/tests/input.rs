use super::*;
    ...
    #[test]
    pub(crate) fn native_input_ctrl_space_opens_spotlight_without_forwarding_space() {
    ...
    #[test]
    pub(crate) fn native_input_visible_spotlight_collects_typed_letters() {
        let mut input = NativeInputState::new(320, 200);
    ...
    #[test]
    pub(crate) fn spotlight_motion_updates_visual_cursor_without_forwarding_client_motion() {
        let mut input = NativeInputState::new(320, 200);
        input.handle_key_event(KEY_LEFTCTRL, 1);
        input.handle_key_event(KEY_SPACE, 1);
    ...
    #[test]
    pub(crate) fn spotlight_locked_motion_does_not_leak_relative_or_absolute_motion() {
        let mut input = NativeInputState::new(320, 200);
    ...
    #[test]
    pub(crate) fn spotlight_client_cursor_repaints_even_with_hardware_cursor_mode() {
        let mut input = NativeInputState::new(320, 200);
        input.handle_key_event(KEY_LEFTCTRL, 1);
        input.handle_key_event(KEY_SPACE, 1);
        let effect = input.handle_pointer_motion_delta(24.0, 12.0);
    ...
    #[test]
    pub(crate) fn ordinary_motion_does_not_apply_compositor_only_position_update() {
        let mut input = NativeInputState::new(320, 200);
        let effect = input.handle_pointer_motion_delta(24.0, 12.0);
    ...
    #[test]
    pub(crate) fn native_input_alt_p_requests_session_exit_without_forwarding_p() {
        let mut input = NativeInputState::new(320, 200);
    ...
    #[test]
    pub(crate) fn native_input_ctrl_c_is_forwarded_to_clients() {
        let mut input = NativeInputState::new(320, 200);
    ...
    #[test]
    pub(crate) fn native_input_alt_mouse_buttons_start_window_interactions() {
        let mut input = NativeInputState::new(320, 200);
        input.handle_key_event(KEY_LEFTALT, 1);
    ...
    #[test]
    pub(crate) fn native_input_unlocked_relative_motion_moves_cursor_and_preserves_relative_delta() {
        let mut input = NativeInputState::new(320, 200);
    ...
    #[test]
    pub(crate) fn native_input_locked_relative_motion_preserves_delta_without_moving_cursor() {
        let mut input = NativeInputState::new(320, 200);
    ...
    #[test]
    pub(crate) fn native_input_locked_absolute_motion_does_not_move_cursor() {
        let mut input = NativeInputState::new(320, 200);
        input.set_pointer_locked_at(input.cursor_position_f64());
    ...
    #[test]
    pub(crate) fn locked_pointer_motion_never_accumulates_absolute_cursor_position() {
    ...
    #[test]
    pub(crate) fn native_input_unlock_restore_sets_logical_cursor_position() {
        let mut input = NativeInputState::new(320, 200);
        input.set_pointer_locked_at(input.cursor_position_f64());
    ...
    #[test]
    pub(crate) fn native_input_first_real_move_after_unlock_starts_from_restored_position() {
        let mut input = NativeInputState::new(320, 200);
        input.set_pointer_locked_at(input.cursor_position_f64());
        input.handle_pointer_motion_delta(200.0, 200.0);
        input.clear_pointer_constraint();
    ...
    #[test]
    pub(crate) fn native_input_confined_relative_motion_clamps_to_region_and_keeps_absolute_motion() {
        let mut input = NativeInputState::new(320, 200);
    ...
    #[test]
    pub(crate) fn native_input_confined_absolute_motion_clamps_and_requests_cursor_repaint() {
        let mut input = NativeInputState::new(320, 200);
        input.set_pointer_confined(OutputRegion::from_rect(
            OutputRect::new(40.0, 30.0, 60.0, 40.0).unwrap(),
        ));
    ...
    #[test]
    pub(crate) fn native_pointer_constraint_backend_activates_locked_once() {
    ...
    #[test]
    pub(crate) fn native_pointer_constraint_backend_activates_confined_with_region() {
        let mut backend = NativePointerConstraintBackend::new();
        let id = PointerConstraintBackendId {
            constraint_id: 8,
            generation: 1,
    ...
    #[test]
    pub(crate) fn native_pointer_constraint_backend_mismatched_deactivation_cannot_unlock_newer_lock() {
        let mut backend = NativePointerConstraintBackend::new();
    ...
    #[test]
    pub(crate) fn native_pointer_constraint_backend_deactivation_restores_hint_or_anchor() {
        let mut backend = NativePointerConstraintBackend::new();
        let id = PointerConstraintBackendId {
            constraint_id: 10,
            generation: 1,
    ...
    #[test]
    pub(crate) fn native_pointer_constraint_backend_preserves_fractional_activation_anchor() {
        let mut backend = NativePointerConstraintBackend::new();
        let id = PointerConstraintBackendId {
            constraint_id: 12,
            generation: 3,
    ...
    #[test]
    pub(crate) fn native_pointer_constraint_backend_confined_deactivation_does_not_restore_anchor() {
        let mut backend = NativePointerConstraintBackend::new();
        let id = PointerConstraintBackendId {
            constraint_id: 11,
            generation: 1,
    ...
    #[test]
    pub(crate) fn native_pointer_constraint_backend_updates_confined_region_in_place() {
        let mut backend = NativePointerConstraintBackend::new();
        let id = PointerConstraintBackendId {
            constraint_id: 12,
            generation: 1,
    ...
    #[test]
    pub(crate) fn native_pointer_constraint_backend_tracks_cursor_visibility_changes() {
        let mut backend = NativePointerConstraintBackend::new();
    ...
    #[test]
    pub(crate) fn native_input_alt_right_starts_window_resize() {
        let mut input = NativeInputState::new(320, 200);
    ...
    #[test]
    pub(crate) fn native_input_alt_release_ends_active_window_interaction() {
        let mut input = NativeInputState::new(320, 200);
        input.handle_key_event(KEY_LEFTALT, 1);
    ...
    #[test]
    pub(crate) fn native_input_alt_keyboard_shortcuts_map_to_window_actions() {
        let mut input = NativeInputState::new(320, 200);
        input.handle_key_event(KEY_LEFTALT, 1);
    ...
    #[test]
    pub(crate) fn native_input_shortcut_inhibition_forwards_window_shortcuts_to_client() {
        let mut input = NativeInputState::new(320, 200);
    ...
    #[test]
    pub(crate) fn native_input_shortcut_inhibition_keeps_emergency_exit_shortcut() {
        let mut input = NativeInputState::new(320, 200);
        input.set_keyboard_shortcuts_inhibited(true);
        input.handle_key_event(KEY_LEFTALT, 1);
    ...
    #[test]
    pub(crate) fn native_input_backend_plan_prefers_libseat_libinput_when_available() {
    ...
    #[test]
    pub(crate) fn native_input_backend_plan_uses_direct_libinput_without_libseat() {
        let plan = NativeInputBackendPlan::choose(NativeInputBackendChoice {
            preference: NativeInputBackendPreference::Auto,
    ...
    #[test]
    pub(crate) fn native_input_backend_plan_can_force_raw_evdev_for_debugging() {
        let plan = NativeInputBackendPlan::choose(NativeInputBackendChoice {
    ...
    #[test]
    pub(crate) fn native_input_backend_plan_falls_back_to_raw_when_libinput_is_unavailable() {
        let plan = NativeInputBackendPlan::choose(NativeInputBackendChoice {
            preference: NativeInputBackendPreference::Auto,
            libseat_available: true,
