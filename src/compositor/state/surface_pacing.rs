...
mod tests {
    ...
    fn submission_safety_is_scoped_to_the_prepared_frame_batch() {
        ...
        let empty_batch = |frame_id, claims| CompositorFrameBatch {
            ...
            presentation_samples_bound: false,
    ...
    #[test]
    fn direct_submission_without_a_claim_ignores_unrelated_future_timing() {
        let mut state = CompositorState::default();
        let sample = state
            .current_commit_timing_clock_sample()
            .expect("test clock sample should be available");
        let unrelated_readiness = CommitTimingReadiness {
            transaction_id: SurfaceTreeTransactionId::new(7),
            requested_not_before: CommitTimingConstraint::from_protocol(u64::MAX, 0).unwrap(),
            selected_monotonic_presentation_time: MonotonicTimestampNs::new(u64::MAX),
            release_for_render_at: MonotonicTimestampNs::new(u64::MAX),
            selected_sequence: 7,
            clock_generation: 1,
            clock_mapping: CommitTimingClockMappingMetadata {
                sample,
                monotonic_not_before: MonotonicTimestampNs::new(u64::MAX),
                is_representable: false,
            },
        };
        state.active_commit_timing_targets.insert(
            700,
            vec![(1, SurfaceCommitSequence::initial(), unrelated_readiness)],
        );
        let direct_batch = CompositorFrameBatchId::new(
            std::num::NonZeroU64::new(70).expect("test batch ID is nonzero"),
        );
        state.frame_batches.insert(
            direct_batch,
    ...
    #[test]
    fn direct_submission_with_its_monotonic_claim_waits_then_proceeds() {
        let mut state = CompositorState::default();
        let sample = state
            .current_commit_timing_clock_sample()
            .expect("test clock sample should be available");
        let selected = MonotonicTimestampNs::new(
            timestamp_as_nanos_u128(sample.monotonic_after) as u64 + 1_000_000,
        );
        let claim = CommitTimingTargetClaim {
            surface_id: 71,
            surface_generation: 1,
            ...
            readiness: CommitTimingReadiness {
                transaction_id: SurfaceTreeTransactionId::new(71),
                requested_not_before: CommitTimingConstraint::from_protocol(0, 0).unwrap(),
                selected_monotonic_presentation_time: selected,
                release_for_render_at: MonotonicTimestampNs::new(0),
                selected_sequence: 71,
                clock_generation: 1,
                clock_mapping: CommitTimingClockMappingMetadata {
                    sample,
                    monotonic_not_before: MonotonicTimestampNs::new(0),
        ...
        let direct_batch = CompositorFrameBatchId::new(
            std::num::NonZeroU64::new(71).expect("test batch ID is nonzero"),
        );
        state.frame_batches.insert(
            direct_batch,
            CompositorFrameBatch {
        ...
        assert!(state.commit_timing_submission_is_safe_for_batch(direct_batch, selected, 1,));
    }

    #[test]
    fn direct_realtime_backward_and_forward_revalidation_share_timing_rules() {
        let requested = CommitTimingConstraint::from_protocol(1001, 0).unwrap();
        let initial_sample = CommitTimingClockSample {
            monotonic_before: PresentationTimestamp::from_microseconds(100, 0).unwrap(),
            monotonic_after: PresentationTimestamp::from_microseconds(100, 0).unwrap(),
        ...
        let initial_deadline = requested.scheduler_deadline(initial_sample);
        let readiness = CommitTimingReadiness {
            transaction_id: SurfaceTreeTransactionId::new(72),
            requested_not_before: requested,
            selected_monotonic_presentation_time: initial_deadline.monotonic_not_before,
            release_for_render_at: MonotonicTimestampNs::new(100_500_000_000),
            selected_sequence: 72,
            clock_generation: 1,
            clock_mapping: CommitTimingClockMappingMetadata {
                sample: initial_sample,
                monotonic_not_before: initial_deadline.monotonic_not_before,
        ...
        assert_eq!(
            revalidate_commit_timing_readiness(
                readiness,
                CommitTimingClockSample {
                    monotonic_before: PresentationTimestamp::from_microseconds(100, 500_000)
                        .unwrap(),
                    monotonic_after: PresentationTimestamp::from_microseconds(100, 500_000)
                        .unwrap(),
                    presentation_now: PresentationTimestamp::from_microseconds(999, 500_000)
                        .unwrap(),
                    presentation_clock: PresentationClock::Realtime,
                },
            ),
