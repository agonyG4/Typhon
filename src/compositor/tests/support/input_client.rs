...
pub(in crate::compositor::tests) fn exercise_cursor_mapping_only_commit(
    socket_path: &PathBuf,
    commands: &Sender<ServerCommand>,
) -> Result<(ClientCursorSnapshot, ClientCursorSnapshot), Box<dyn std::error::Error>> {
    let stream = UnixStream::connect(socket_path)?;
    ...
    let cursor_surface = compositor.create_surface(&qh, ());
    pointer.set_cursor(serial, Some(&cursor_surface), 1, 1);
    let buffer = attach_test_buffered_surface(&cursor_surface, &shm, &qh, 24, 24)?;
    cursor_surface.commit();
    connection.flush()?;
    wait_for_server_commands(commands);
    queue.roundtrip(&mut state)?;
    ...
    cursor_surface.set_buffer_transform(client_wl_output::Transform::_180);
    cursor_surface.commit();
    connection.flush()?;
    wait_for_server_commands(commands);
    queue.roundtrip(&mut state)?;
