...
#[test]
fn runtime_layout_navigation_publishes_wrapped_groups_without_key_events() {
    let _guard = KEYBOARD_LAYOUT_ENV_LOCK.lock().unwrap();
    let previous_layout = std::env::var_os("OBLIVION_ONE_XKB_LAYOUT");
    let previous_variant = std::env::var_os("OBLIVION_ONE_XKB_VARIANT");
    let previous_options = std::env::var_os("OBLIVION_ONE_XKB_OPTIONS");
    ...
    let mut navigate = |direction: Direction| {
        let (reply, result) = mpsc::channel();
        let command = match direction {
            Direction::Next => ServerCommand::NextKeyboardLayout { reply },
            Direction::Previous => ServerCommand::PreviousKeyboardLayout { reply },
        };
    ...
    assert_eq!(state.keyboard_groups, vec![0, 1, 0, 1]);
    assert_eq!(state.keyboard_keys, Vec::<u32>::new());
    assert_eq!(
        state
            .keyboard_event_log
...
fn wl_pointer_v4_receives_only_legacy_axis_events() {
    ...
    let state = create_focused_toplevel_and_receive_pointer_axis_at_version(
        &socket_path,
        &commands,
        4,
        Some(30),
...
#[test]
fn wl_pointer_v8_does_not_receive_zero_v120() {
    let socket_name = unique_socket_name();
    let server = OwnCompositorServer::bind(&socket_name).unwrap();
    let socket_path = runtime_socket_path(&socket_name);
    let (commands, server_thread) = spawn_controllable_test_server(server);
    ...
    let state = state.unwrap();
    assert!(state.pointer_axis_value120.is_empty());
}
